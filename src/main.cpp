      ...
      if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
        ...
        if (s != "") {
          ...
          if (event == "telemetry") {
            ...
            *
            */
            // a. Polynomial coefficients of target trajectory in vehicle frame:
            Eigen::VectorXd coeffs = polyfit(
              trajX,
              trajY
            );
            // a. Polynomial derivate coefficients:
            ...
            double psiVeh = 0.0;
            double vVeh = v;
            double  cte = polyeval(coeffs,xVeh) - yVeh;
            double epsi = psiVeh - atan(polyeval(coeffsDiff, xVeh));
            double steer = j[1]["steering_angle"];
            double acc = j[1]["throttle"];

            state << xVeh, yVeh, psiVeh, vVeh, cte, epsi, steer, acc;

            // c. Identify optimal control:
            double delay = 0.1;
            auto result = mpc.Solve(state, coeffs, delay);
            ...
            for (size_t i = 2; i < result.size(); i += 2) {
              ...
              next_x_vals.push_back(result[i]);
              next_y_vals.push_back(polyeval(coeffs, result[i]));
            }
